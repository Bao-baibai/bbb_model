...
class CrossTransformer(nn.Module):
    def __init__(self, *, source_num_frames, tgt_num_frames, dim, depth, heads, mlp_dim, pool = 'cls', dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        # 降维线性层，用于在拼接前调整 source_x 的维度
        self.dim_adjust = nn.Linear(dim * 2, dim)  # 将 256 降维到 128
        self.dim_adjust_target = nn.Linear(dim * 2, dim)  # 将 256 降维到 128
        self.pool = pool
    ...
    def forward(self, source_x, target_x):
        ...
        if extra_token.size(-1) != source_x.size(-1):
            source_x = self.dim_adjust(source_x)

            # 拼接\
        if target_x.size(-1) == 256:  # 检查是否需要降维
            target_x = self.dim_adjust_target(target_x)
